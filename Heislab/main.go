...
func main() {
	...
	for {
		switch state {
		case 0:
			Slave.Slave_init(chan_slave_received_msg, chan_master_bcast, BCAST_PORT, chan_error_slave, chan_change_to_master, &MasterIP, state, chan_new_network_order, chan_new_hw_order, chan_new_master_order, chan_order_executed, chan_set_lights, chan_network_lights, PORT)
		...
		case 1:
			Master.Master_init(chan_master, chan_master_received_msg, chan_is_alive, PORT, chan_change_to_slave, chan_master_ip, chan_new_hw_order, chan_new_master_order, chan_source_ip, chan_set_lights, chan_order_executed, chan_state, chan_dir, chan_floor, chan_elev_state, chan_reset, chan_descendant_nr, chan_slavelist)
			for {
				select {
