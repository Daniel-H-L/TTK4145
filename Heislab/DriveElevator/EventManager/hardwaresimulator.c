...
This module is a hack to make the tripcounter code compile and run. It
is not part of the tripcounter code ment to be read or understood.
...
void * windowThread(void * threadData /*Unused*/) {
  ...
  XSelectInput(display, win, ExposureMask | KeyPressMask | ButtonReleaseMask | ButtonPressMask | StructureNotifyMask);
  ...
  while ( 1 ) {
    ...
    switch ( report.type ) {
	  ...
	  if(report.xbutton.button == 3) button2 = 0;
        ...
        case KeyPress:
          printf("keypressevent: state = %d button = %d \n",report.xkey.state,report.xkey.keycode);
          printf("Quitting because of keypress! Do not press a key if you do not want to quit.\n\n");
