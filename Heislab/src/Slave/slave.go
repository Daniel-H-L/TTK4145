...
func Slave_init(portNr string, chan_change_to_master chan bool, chan_kill chan bool, chan_kill2 chan bool) {
	*Master_IP = ""

	chan_received_msg := make(chan Network.StandardData, 1)

	go Listen_for_master(chan_received_msg, portNr, chan_change_to_master, chan_kill2)

	fmt.Println("In slave init...")

	go Slave_test_drive(chan_kill)
}

func Listen_for_master(portNr string, chan_change_to_master chan bool, chan_kill chan bool) {

	chan_received_msg := make(chan Network.StandardData)
...
func Slave_test_drive(Master_IP *string, port_nr string, chan_kill chan bool) {

	chan_new_hw_order := make(chan DriveElevator.Button, 100)
	chan_new_master_order := make(chan DriveElevator.Button, 100)
	chan_kill_network := make(chan bool, 1)
	chan_order_executed := make(chan int, 100)
	chan_set_lights := make(chan [3][4]int, 100)

	chan_source_ip := make(chan string, 1)
	chan_descendant_nr := make(chan int, 1)
	chan_new_network_order := make(chan Network.NewOrder, 100)
